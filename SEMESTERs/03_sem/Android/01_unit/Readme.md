...
- [ ] Write a base class `Vehicle` and extend it with `Car`, overriding a `start()` method.
- [ ] Create a encapsulated `User` POJO with private fields (`id`, `name`, `email`) and accessors.
- [ ] Practice instantiating objects dynamically and passing them into methods.

### 2. Interfaces & Callbacks
- [ ] Create a custom `OnClickListener` interface with an `onClick()` contract.
- [ ] Build a `Button` class that triggers the listener method.
- [ ] Pass an **anonymous inner class** to set the click behavior dynamically.

### 3. Collections Framework
- [ ] Initialize an `ArrayList<User>` and write methods to search, add, and remove items.
- [ ] Use a `HashMap<String, Integer>` to store items and look up values by unique keys.
- [ ] Iterate through a dataset using enhanced `for-each` loops.

### 4. Exception & Null Safety
- [ ] Write a method that takes string input, converts it to an integer using `Integer.parseInt()`, and safely catches `NumberFormatException`.
- [ ] Practice defensive null checks before calling methods on potentially null objects.

### 5. Multithreading & Asynchrony
- [ ] Create a basic background task using `new Thread(new Runnable() { ... }).start()`.
- [ ] Print statements from both the main thread and background thread to observe execution order.
